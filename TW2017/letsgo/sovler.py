def base64decode(s):
    b64s = "WPnq7JEM2AskwjoX3VRx9aHbiYfd4#Zp05TUGc1SBCFNgvhz8rQl6@ytIKumDeOL"
    b64p = "="
    ret = ""
    s2 = s.replace(b64p, "")
    left = 0
...
def decrypt(enc, key):
	...
	for i in range(0x20):
		for j in range(7, -1, -1):
			value = enc[(j + 1) & 7]
			value = ror(value, 1, 8)
			enc[(j + 1) & 7] = (value - s_box[(enc[j] + key[j]) & 0xff]) & 0xff
	return str(enc)
...
key = base64decode('R6aYb@VboTG==')	# KEY_TW:)
flag = ''
